...
import { AppText, Badge, Button, Card, EmptyState, Icon } from '@/components/ui';
import { Bill, BillImport, Resident, ThemePreference, useApp } from '@/context/app-context';

const icons = {
  drop: { ios: 'drop.fill', android: 'water_drop', web: 'water_drop' } as any,
  home: { ios: 'square.grid.2x2', android: 'dashboard', web: 'dashboard' } as any,
  bills: { ios: 'doc.text', android: 'receipt_long', web: 'receipt_long' } as any,
  users: { ios: 'person.2', android: 'group', web: 'group' } as any,
  approvals: { ios: 'person.badge.clock', android: 'how_to_reg', web: 'how_to_reg' } as any,
  upload: { ios: 'arrow.up.doc', android: 'upload_file', web: 'upload_file' } as any,
  settings: { ios: 'gearshape', android: 'settings', web: 'settings' } as any,
  logout: { ios: 'rectangle.portrait.and.arrow.right', android: 'logout', web: 'logout' } as any,
  sun: { ios: 'sun.max', android: 'light_mode', web: 'light_mode' } as any,
  moon: { ios: 'moon', android: 'dark_mode', web: 'dark_mode' } as any,
  system: { ios: 'circle.lefthalf.filled', android: 'contrast', web: 'contrast' } as any,
  check: { ios: 'checkmark', android: 'check', web: 'check' } as any,
  close: { ios: 'xmark', android: 'close', web: 'close' } as any,
  card: { ios: 'creditcard', android: 'credit_card', web: 'credit_card' } as any,
  meter: { ios: 'gauge.with.dots.needle.67percent', android: 'speed', web: 'speed' } as any,
  calendar: { ios: 'calendar', android: 'calendar_month', web: 'calendar_month' } as any,
  info: { ios: 'info.circle', android: 'info', web: 'info' } as any,
  menu: { ios: 'line.3.horizontal', android: 'menu', web: 'menu' } as any,
};

type Page = 'overview' | 'bills' | 'approvals' | 'upload' | 'residents' | 'settings';
    ...
    <SafeAreaView style={[styles.root, { backgroundColor: colors.background }]}>
      <View style={[styles.shell, !desktop && styles.shellMobile]}>
            ...
            <View style={[styles.userBlock, { borderTopColor: colors.border }]}>
              ...
              <View style={styles.userCopy}>
                ...
                <AppText size={11} muted>
                  {isAdmin ? 'HOA Administrator' : `Block ${currentUser?.block}, Lot ${currentUser?.lot}`}
                </AppText>
              </View>
              <Pressable accessibilityLabel="Sign out" onPress={signOut} style={styles.iconPress}>
        ...
        <View style={styles.main}>
          ...
          <ScrollView
            contentContainerStyle={[styles.content, !desktop && styles.contentMobile]}
            showsVerticalScrollIndicator={false}>
            {isAdmin ? <AdminPage page={page} setPage={setPage} /> : <ResidentPage page={page} />}
          </ScrollView>
        </View>
      </View>
    </SafeAreaView>
...
function NavItem({
  ...
  return (
    <Pressable
      ...
      <Icon name={icon} size={19} color={active ? colors.primary : colors.textMuted} />
      <AppText
        size={13}
      ...
      {currentBill ? (
        <Card style={styles.currentBill}>
          ...
          <View style={[styles.readings, { borderTopColor: colors.border }]}>
            <Reading label="Previous" value={`${currentBill.previousReading} m³`} />
            <Reading label="Current" value={`${currentBill.currentReading} m³`} />
            <Reading label="Used" value={`${currentBill.consumption} m³`} strong />
            <Button
              label="Pay now"
              icon={icons.card}
...
function BillingHistory({ resident }: { resident: Resident }) {
  ...
  return (
    <>
      <PageHeader title="Billing history" subtitle="Review all statements and payment records." />
...
function BillTable({ bills, onPay }: { bills: Bill[]; onPay: (bill: Bill) => void }) {
  const { colors } = useApp();
  return (
    <Card style={styles.tableCard}>
      {bills.length === 0 ? (
        <EmptyState icon={icons.bills} title="No statements yet" body="New bills will appear here." />
      ) : (
        bills.map((bill, index) => (
          <View
            ...
            style={[
              styles.billRow,
              index < bills.length - 1 && { borderBottomColor: colors.border, borderBottomWidth: 1 },
            ]}>
            <View style={styles.billMain}>
              <View style={[styles.billIcon, { backgroundColor: colors.surfaceAlt }]}>
            ...
            {bill.status === 'paid' ? (
              ...
              <Button label="Pay" onPress={() => onPay(bill)} style={styles.smallButton} />
            )}
          </View>
        ))
      )}
...
function PaymentModal({ bill, onClose }: { bill: Bill | null; onClose: () => void }) {
        ...
        <Card style={styles.modalCard}>
          {paid ? (
            <EmptyState
              ...
              action={<Button label="Done" onPress={close} style={styles.modalAction} />}
            />
          ) : (
            <>
              ...
              <Button label="Complete dummy payment" icon={icons.card} onPress={complete} />
            </>
          )}
        </Card>
      </View>
...
function AdminPage({ page, setPage }: { page: Page; setPage: (page: Page) => void }) {
...
function AdminOverview({ setPage }: { setPage: (page: Page) => void }) {
  ...
  const approved = residents.filter(
    (resident) => resident.role === 'resident' && resident.status === 'approved',
  ).length;
  ...
  return (
    <>
      <PageHeader title="Community overview" subtitle="Water billing operations at a glance." />
      <View style={styles.statGrid}>
        <StatCard label="Approved residents" value={String(approved)} icon={icons.users} tone="primary" />
        <StatCard label="Pending approvals" value={String(pending)} icon={icons.approvals} tone="warning" />
        <StatCard label="Outstanding" value={peso(outstanding)} icon={icons.card} tone="neutral" />
        <StatCard label="Bills issued" value={String(bills.length)} icon={icons.bills} tone="success" />
      </View>
      <View style={styles.adminActions}>
        <Card style={styles.actionCard}>
          <View style={styles.actionCopy}>
            <View>
              <Badge label={`${pending} pending`} tone={pending ? 'warning' : 'success'} />
              <AppText size={19} weight="800" style={styles.actionTitle}>
                Review registrations
              </AppText>
              <AppText muted>Verify household information and grant resident access.</AppText>
            </View>
            <Button
        ...
        <Card style={styles.actionCard}>
          <View style={styles.actionCopy}>
            <View>
              ...
              <AppText muted>Upload a prepared workbook and issue statements in bulk.</AppText>
            </View>
            <Button
      ...
      <ResidentList residents={residents.filter((item) => item.role === 'resident').slice(-4).reverse()} />
    </>
  );
}
...
function Approvals() {
  ...
  return (
    <>
      <PageHeader title="Registration approvals" subtitle="Verify resident details before granting access." />
      {pending.length === 0 ? (
        <Card>
          <EmptyState
            icon={icons.check}
          ...
          {pending.map((resident) => (
            <Card key={resident.id} style={styles.approvalCard}>
              ...
              <View style={styles.detailsGrid}>
                ...
                <Detail label="Address" value={`Block ${resident.block}, Lot ${resident.lot}, ${resident.street}`} />
                <Detail label="Recent bill ID" value={resident.recentBillingId} />
              </View>
...
function BillUpload() {
  ...
  async function chooseFile() {
    ...
    try {
      ...
      setMessage('The workbook could not be read. Use a valid .xlsx or .xls file.');
    } finally {
      setLoading(false);
    }
  }
  ...
  return (
    <>
      <PageHeader title="Import water bills" subtitle="Issue resident statements from an Excel workbook." />
      <Card>
        <View style={styles.uploadLayout}>
          <View style={[styles.uploadZone, { borderColor: colors.border, backgroundColor: colors.surfaceAlt }]}>
          ...
          <View style={styles.template}>
            ...
            <AppText size={13} muted>
              The first worksheet must include these headers:
            </AppText>
            {[
              'email',
              'billingId',
        ...
        {message ? (
          <View style={[styles.importMessage, { backgroundColor: colors.primarySoft }]}>
      ...
      {rows.length ? (
        <>
          <View style={styles.sectionHeading}>
            ...
            <Button label={`Import ${rows.length} bills`} icon={icons.check} onPress={commit} />
          </View>
          <Card style={styles.tableCard}>
...
function Residents() {
  ...
  return (
    <>
      <PageHeader title="Residents" subtitle="All registered households and account statuses." />
    ...
    <Card style={styles.tableCard}>
      {residents.map((resident, index) => (
        <View
          ...
          style={[
            styles.residentRow,
            index < residents.length - 1 && { borderBottomWidth: 1, borderBottomColor: colors.border },
          ]}>
          <View style={[styles.avatar, { backgroundColor: colors.primarySoft }]}>
            <AppText weight="800" style={{ color: colors.primary }}>
...
function Settings() {
  ...
  return (
    <>
      <PageHeader title="Settings" subtitle="Manage appearance and review account information." />
          ...
          {options.map((option) => {
            ...
            return (
              <Pressable
                ...
                <Icon name={option.icon} color={active ? colors.primary : colors.textMuted} />
                <AppText
                  weight={active ? '700' : '500'}
      ...
      <Card style={styles.settingsCard}>
        ...
        <View style={styles.detailsGrid}>
          ...
          <Detail
            label="Access"
            value={currentUser?.role === 'admin' ? 'HOA Administrator' : 'Approved resident'}
          />
        </View>
      </Card>
    </>
...
function normalizeBillRow(row: SheetRow): BillImport | null {
  ...
  if (typeof dueValue === 'number') {
    ...
    if (parsed) dueDate = `${parsed.y}-${String(parsed.m).padStart(2, '0')}-${String(parsed.d).padStart(2, '0')}`;
  }
  if (
    !email ||
    !billingId ||
...
const styles = StyleSheet.create({
  ...
  brand: { height: 70, flexDirection: 'row', alignItems: 'center', gap: 11, paddingHorizontal: 4 },
  brandCompact: { height: 'auto' },
  logo: { width: 42, height: 42, borderRadius: 7, alignItems: 'center', justifyContent: 'center' },
  nav: { flex: 1, gap: 5, paddingTop: 18 },
  navItem: { height: 46, borderRadius: 7, flexDirection: 'row', alignItems: 'center', gap: 12, paddingHorizontal: 13 },
  navItemCompact: { height: 42, paddingHorizontal: 12 },
  userBlock: { borderTopWidth: 1, paddingTop: 16, flexDirection: 'row', alignItems: 'center', gap: 10 },
  avatar: { width: 38, height: 38, borderRadius: 19, alignItems: 'center', justifyContent: 'center' },
  userCopy: { flex: 1, minWidth: 0 },
  iconPress: { width: 38, height: 38, alignItems: 'center', justifyContent: 'center', borderRadius: 7 },
  mobileHeader: { minHeight: 64, borderBottomWidth: 1, paddingHorizontal: 16, flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' },
  main: { flex: 1 },
  mobileNav: { flexGrow: 0, borderBottomWidth: 1 },
  mobileNavContent: { paddingHorizontal: 10, paddingVertical: 7, gap: 4 },
  content: { width: '100%', maxWidth: 1180, alignSelf: 'center', padding: 34, gap: 22 },
  ...
  statIcon: { width: 38, height: 38, borderRadius: 7, alignItems: 'center', justifyContent: 'center', marginBottom: 3 },
  currentBill: { gap: 20 },
  billHeadline: { flexDirection: 'row', flexWrap: 'wrap', justifyContent: 'space-between', gap: 20 },
  billTitle: { gap: 5 },
  amountBlock: { alignItems: 'flex-end' },
  readings: { borderTopWidth: 1, paddingTop: 18, flexDirection: 'row', flexWrap: 'wrap', alignItems: 'center', gap: 34 },
  payButton: { marginLeft: 'auto', minWidth: 130 },
  sectionHeading: { flexDirection: 'row', flexWrap: 'wrap', justifyContent: 'space-between', alignItems: 'center', gap: 12, marginTop: 4 },
  tableCard: { paddingVertical: 3, paddingHorizontal: 18 },
  billRow: { minHeight: 74, flexDirection: 'row', flexWrap: 'wrap', alignItems: 'center', gap: 18, paddingVertical: 12 },
  billMain: { flex: 1, minWidth: 180, flexDirection: 'row', alignItems: 'center', gap: 12 },
  billIcon: { width: 38, height: 38, borderRadius: 7, alignItems: 'center', justifyContent: 'center' },
  billDue: { minWidth: 130, alignItems: 'flex-end' },
  smallButton: { height: 36, minWidth: 76 },
  modalBackdrop: { flex: 1, backgroundColor: 'rgba(3, 12, 14, 0.62)', alignItems: 'center', justifyContent: 'center', padding: 20 },
  modalCard: { width: '100%', maxWidth: 440, gap: 18, padding: 24 },
  modalTop: { flexDirection: 'row', justifyContent: 'space-between', gap: 12 },
  paymentSummary: { borderRadius: 7, padding: 20, alignItems: 'center', gap: 3 },
  notice: { borderRadius: 7, padding: 12, flexDirection: 'row', gap: 9, alignItems: 'center' },
  ...
  approvalGrid: { gap: 14 },
  approvalCard: { gap: 20 },
  residentHeading: { flexDirection: 'row', flexWrap: 'wrap', justifyContent: 'space-between', gap: 12 },
  ...
  uploadZone: { flex: 1.5, minWidth: 260, minHeight: 300, borderWidth: 1, borderStyle: 'dashed', borderRadius: 8, alignItems: 'center', justifyContent: 'center', padding: 24, gap: 9 },
  uploadIcon: { width: 58, height: 58, borderRadius: 29, alignItems: 'center', justifyContent: 'center', marginBottom: 4 },
  ...
  residentRow: { minHeight: 72, flexDirection: 'row', flexWrap: 'wrap', alignItems: 'center', gap: 12, paddingVertical: 11 },
  residentMain: { flex: 1, minWidth: 180 },
  residentAddress: { minWidth: 210 },
  settingsCard: { gap: 12 },
  themeOptions: { flexDirection: 'row', flexWrap: 'wrap', gap: 10, marginTop: 4 },
  themeOption: { minWidth: 130, height: 52, borderWidth: 1, borderRadius: 7, paddingHorizontal: 15, flexDirection: 'row', alignItems: 'center', gap: 10 },
